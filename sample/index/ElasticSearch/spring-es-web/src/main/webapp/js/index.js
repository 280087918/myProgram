			...
			if (event.keyCode == 38) {
				if (current.length > 0) {
					...
					if (prevLi.length > 0) {
						...
						input.val(replaceHtml(prevLi.html()));
					}
				} else {
					var last = suggestWrap.find('li:last');
					last.addClass('hover');
					input.val(replaceHtml(last.html()));
				}
				...
				if (current.length > 0) {
					...
					if (nextLi.length > 0) {
						...
						input.val(replaceHtml(nextLi.html()));
					}
				} else {
					var first = suggestWrap.find('li:first');
					first.addClass('hover');
					input.val(replaceHtml(first.html()));
				}
			}
...
function sendKeyWordToBack(keyword) {
	...
	$.ajax({
		type : "POST",
		url : $('#basePath').val() + "/searchTips.sc",
		async : false,
		data : obj,
		dataType : "json",
		success : function(data) {
			if(!$.isEmptyObject(data)) {
				...
				var bData = []
				searchSuggest.dataDisplay(bData);
			}
		}
	});
				...
				if(!$.isEmptyObject(data)) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						htmlStr += ('<div class="unit">' +
										...
										'<div class="price">￥ ' + data[i].salesPrice + '</div>' +
									'</div>'
								)
					}
					console.log(json);
