...
1.首先在那三个服务器都安装上rabbitmq，并通过网页登录看是否能顺利开启并登陆
  http://192.168.2.181:15672
  http://192.168.2.182:15672
  http://192.168.2.183:15672
...
3.定位好这个文件以后，我们需要做以这个操作，将主mq的.erlang.cookie复制到另外两台机器里面
  ...
  b)在181上将/root/.erlang.cookie复制到182和183上
    181上执行
    scp /root/.erlang.cookie root@192.168.2.182:/root
    scp /root/.erlang.cookie root@192.168.2.183:/root
...
5.配置host,为了方便直接将下面配置复制到主还有所有节点当中
  vim /etc/hosts
  192.168.2.181 rabbitmq_node1
  192.168.2.182 rabbitmq_node2
  192.168.2.183 rabbitmq_node3

  ssh客户端必须重新连接linux服务器，直到命令行开头这么显示:
  [root@rabbitmq_node2 ~]# 
  那么下面就已node1，node2，node3来表述这些机器
...
10.使用HAProxy做负载均衡器
  首先负载均衡器放到192.168.2.184上面，所以下面对于haproxy的操作都是在184上操作的
...
11.编辑haproxy配置文件
   ...
   mode tcp
   balance     roundrobin
   server  rabbitmq_node1 192.168.2.181:5672 check inter 2000 rise 2 fall 3
   server  rabbitmq_node2 192.168.2.182:5672 check inter 2000 rise 2 fall 3
   server  rabbitmq_node3 192.168.2.183:5672 check inter 2000 rise 2 fall 3
