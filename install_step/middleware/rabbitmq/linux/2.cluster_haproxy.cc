...
11.编辑haproxy配置文件
   ...
   mode tcp
   balance     roundrobin
   server  vm1 192.168.22.181:5672 check inter 2000 rise 2 fall 3
   server  vm2 192.168.22.182:5672 check inter 2000 rise 2 fall 3
   server  vm3 192.168.22.183:5672 check inter 2000 rise 2 fall 3
...
========================集群延伸==============================
因为haproxy机器本身也有宕机的可能性
解决方案是:再加一台机器，安装haproxy,在两台机器上做vip的漂移(lvs+keepalived)，
Ps.额外添加一台虚拟机192.168.22.185

1.前提准备好185，并且安装好haproxy
      参考上面步骤

2.184和185都安装好keepalived
      yum install -y keepalived

3.184和185上面新增文件
      vim /etc/keepalived/haproxy_down.sh
      输入下面代码
      #!/bin/bash
      pkill keepalived
      # chmod +x /etc/keepalived/haproxy_down.sh #授权可执行权限

4.修改184下的/etc/keepalived/keepalived.conf
	...
	184输入同目录下的2.cluster_haproxy_keepalived_184.conf
	185输入同目录下的2.cluster_haproxy_keepalived_185.conf
